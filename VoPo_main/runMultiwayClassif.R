...
#Date: April 13, 2020
#Dependency: please also install package igraph
#Author: Natalie Stanley stanleyn@stanford.edu
runMultiwayClassif=function(FuncDF,Y,FPV,IterNumClus,propTrain,numPerm,sampID,numCore){
...
#inputs:
	...
	#Y: the response variable with multiple classes. Must be a factor!
...
#Output:
	#StoreNMIs: the vector of MSEs corresponding to each cross validation trial
...
ans=c()
for(i in 1:nrow(ansMat)){
relRow=ansMat[i,]
tabVec=table(relRow)
relVal=names(which.max(table(relRow)))
ans=c(ans,relVal)
}

#calculate the mean squared error between true and actual predictions
r=compare(as.numeric(ans),as.numeric(Y),method='nmi')
StoreNMIs=c(StoreNMIs,r)
} #for ss
StoreNMIs
} #function end
